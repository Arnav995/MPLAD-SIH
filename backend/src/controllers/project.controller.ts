...
export async function getProjectsController(
  ...
  try {
    ...
    const pageSize = getNumberQuery(req.query.page_size) ?? 10;

    const result = await findProjects({
      district: getStringQuery(req.query.district),
      ...
      minRiskIndex: getNumberQuery(req.query.min_risk_index),
...
export async function getProjectController(
  ...
  try {
    const workId = parseWorkId(req.params.workId);
    const project = await findProjectById(workId);
...
function getNumberQuery(value: unknown): number | undefined {
  ...
  if (!Number.isFinite(parsed)) {
    throw new Error("Query parameter must be a valid number");
  }
...
function parseWorkId(
  ...
  if (idStr === undefined || !/^\d+$/.test(idStr)) {
