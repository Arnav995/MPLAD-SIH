...
export class EsakshiService {
  ...
  const instance = axios.create({
    ...
    timeout: Number(process.env.ESAKSHI_TIMEOUT_MS ?? 30000),
  ...
  const wrapped = wrapper(instance as any);

  (wrapped.defaults as any).jar = jar;
  (wrapped.defaults as any).withCredentials = true;

  this.client = wrapped as AxiosInstance;
}
